...
memberRouter.post('/signup', async (req, res) => {
    let apiLogEntry = null;
    try {
        const { mem_id_view, mem_name, mem_nickname, mem_password, mem_img, mem_pnumber, mem_email, mem_sex, mem_age, mes_id } = req.body;
        // 필수값 검증
...
memberRouter.post('/pwcheck', async (req, res) => {
    ...
    if (!user || !user.MEM_ID_VIEW)
        return res.status(401).json({ message: "로그인 세션이 만료되었습니다." });
    try {
...
memberRouter.post('/analyzeWorkout', async (req, res) => {
    try {
        ...
        const recordText = workoutDetails.map((r) => `- ${r.WOO_NAME}(부위: ${r.WOO_TYPE}): 실제 수행 ${r.WOD_COUNT}${r.WOO_UNIT}, 정확도 ${r.WOD_ACCURACY}%`).join("\n");
    ...
    `;
        ...
        if (finalIntensity.includes("강하게"))
            finalIntensity = "강하게";
        else if (finalIntensity.includes("가볍게"))
            finalIntensity = "가볍게";
        else if (finalIntensity.includes("보통"))
            finalIntensity = "보통";
