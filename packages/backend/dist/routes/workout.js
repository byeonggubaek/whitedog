...
import { completeWorkoutRecord, getLatestFinishedWorkoutId, getWorkoutDetails, getWorkoutHistory, getWorkoutRecords, getWorkoutRecordsByPivot, getWorkoutRecordsWithPlan, getWorkoutRecordsWithPlanByPivot, getWorkouts, initWorkoutRecord } from '../db.js';
const workoutRouter = express.Router();
workoutRouter.get('/getLatestFinishedWorkoutId', async (req, res) => {
...
workoutRouter.get('/getWorkoutRecords', async (req, res) => {
    let apiLogEntry = null;
    try {
        const { mem_id, from_dt, to_dt } = req.query;
        if (!mem_id || mem_id === "") {
            return res.status(400).json({ success: false, error: '회원정보 파라미터가 누락되었습니다.' });
        }
        if (!from_dt || !to_dt) {
...
workoutRouter.get('/getWorkoutRecordsByPivot', async (req, res) => {
    let apiLogEntry = null;
    try {
        const { mem_id } = req.query;
...
workoutRouter.get('/getWorkoutRecordsWithPlan', async (req, res) => {
    let apiLogEntry = null;
    try {
        const { mem_id, from_dt, to_dt } = req.query;
        // 필수 파라미터 체크
...
workoutRouter.get('/getWorkoutRecordsWithPlanByPivot', async (req, res) => {
    let apiLogEntry = null;
    try {
        const { mem_id } = req.query;
        const { from_dt } = req.query;
...
workoutRouter.post('/complete', async (req, res) => {
    ...
    try {
        ...
        const earnedPoint = await completeWorkoutRecord(Number(mem_id), Number(wor_id), Number(count), Number(duration), Number(accuracy), Number(woo_id));
